...
//
      if (ifldnum <= 0) {
        grib_compat_printf("g2_getfld: Request for field number must be positive.\n");
        ierr=3;
        return(ierr);
      }
//
...
//
      ...
      if (istart == -1) {
        grib_compat_printf("g2_getfld:  Beginning characters GRIB not found.\n");
        ierr=1;
        return(ierr);
      }
//
...
//
      if (ver != 2) {
        grib_compat_printf("g2_getfld: can only decode GRIB edition 2.\n");
        ierr=2;
        return(ierr);
      }
//
      ...
      for (;;) {
        ...
        if (cgrib[ipos]=='7' && cgrib[ipos+1]=='7' && cgrib[ipos+2]=='7' &&
          ...
          if (ipos != (istart+lengrib)) {
            grib_compat_printf("g2_getfld: '7777' found, but not where expected.\n");
            ierr=4;
            return(ierr);
          }
          break;
        ...
        if ( isecnum<1 || isecnum>7 ) {
          grib_compat_printf("g2_getfld: Unrecognized Section Encountered=%ld\n",isecnum);
          ierr=8;
          return(ierr);
        }
        //
            ...
            if (jerr == 0) {
              ...
              if (lgfld->ibmap == 254)     // use previously specified bitmap
                 if( bmpsave!=0 )
                    ...
                    grib_compat_printf("g2_getfld: Prev bit-map specified, but none exist.\n");
                    ierr=17;
                    return(ierr);
                 }
              else                         // get rid of it
        ...
        if (isecnum==7 && numfld==ifldnum && unpack) {
          ...
          if (jerr == 0) {
            ...
            grib_compat_printf("g2_getfld: return from g2_unpack7 = %d \n",(int)jerr);
            ierr=14;
            return(ierr);
          }
        }
        ...
        if (ipos > (istart+lengrib)) {
          grib_compat_printf("g2_getfld: '7777'  not found at end of GRIB message.\n");
          ierr=7;
          return(ierr);
        }
        //
...
//
      grib_compat_printf("g2_getfld: GRIB message contained %ld different fields.\n",numfld);
      grib_compat_printf("g2_getfld: The request was for field %ld.\n",ifldnum);
      ierr=6;
