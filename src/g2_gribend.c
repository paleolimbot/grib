...
//
      if ( cgrib[0]!=G || cgrib[1]!=R || cgrib[2]!=I || cgrib[3]!=B ) {
        grib_compat_printf("g2_gribend: GRIB not found in given message.\n");
        ierr=-1;
        return (ierr);
      }
//
...
//
      ...
      //    total length, then there is a problem.
        if ( len > lencurr ) {
          grib_compat_printf("g2_gribend: Section byte counts don''t add to total.\n");
          grib_compat_printf("g2_gribend: Sum of section byte counts = %d\n",(int)len);
          grib_compat_printf("g2_gribend: Total byte count in Section 0 = %d\n",(int)lencurr);
          ierr=-3;
          return (ierr);
        }
      }
...
//
      if ( isecnum != 7 ) {
        grib_compat_printf("g2_gribend: Section 8 can only be added after Section 7.\n");
        grib_compat_printf("g2_gribend: Section %ld was the last found in given GRIB message.\n",isecnum);
        ierr=-4;
        return (ierr);
      }
//
