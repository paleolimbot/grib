      ...
      *iofst=*iofst+8;
      ...
      if (idrsnum == 0)
        ...
        if ( igdsnum>=50 && igdsnum <=53 )
          ...
          grib_compat_eprintf("g2_unpack7: Cannot use GDT 3.%d to unpack Data Section 5.51.\n",(int)igdsnum);
          ierr=5;
          if ( lfld != 0 ) free(lfld);
          *fld=0;     //NULL
          return(ierr);
        ...
        grib_compat_eprintf("g2_unpack7: Data Representation Template 5.%d not yet implemented.\n",(int)idrsnum);
        ierr=4;
        if ( lfld != 0 ) free(lfld);
        *fld=0;     //NULL
        return(ierr);
